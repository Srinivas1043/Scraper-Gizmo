...
# def scrape_google_maps(playwright: Playwright, headless: bool = False) -> str:
...
#         html = scrape_google_maps(playwright, headless=False)



from playwright.sync_api import sync_playwright
import pandas as pd
from gmaps_playwright_scraper.scraper import GoogleMapsScraper
from gmaps_playwright_scraper.config import CHROME_EXECUTABLE_PATH, USER_AGENT, VIEWPORT

def scrape_google_maps():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True, slow_mo=100, executable_path=CHROME_EXECUTABLE_PATH)
        context = browser.new_context(viewport=VIEWPORT, user_agent=USER_AGENT)
        page = context.new_page()

        df = pd.DataFrame([{
            'search_query': 'Vegetarian Restaurants near Bangalore, Bellandur',
            'UserName': 'John Doe'
        }])

        scraper = GoogleMapsScraper(page)
        results = scraper.parse_overview_data(df)
        print(results)
        browser.close()
